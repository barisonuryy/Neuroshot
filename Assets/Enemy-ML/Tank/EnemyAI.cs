...
public class EnemyAI : MonoBehaviour
{
    ...
    IEnumerator Fire()
    {
        if (currentAmmo > 0)
        {
            ...
            if (player != null)
            {
                ...
                SpawnProjectile(firePointLeft.position, firePointLeft.rotation, player.GetChild(0).GetChild(0).GetChild(2).GetChild(0).GetChild(0).position);
                SpawnProjectile(firePointRight.position, firePointRight.rotation, player.GetChild(0).GetChild(0).GetChild(2).GetChild(0).GetChild(0).position);
            }
            currentAmmo--;
        }
        else
    ...
    void SpawnProjectile(Vector3 position, Quaternion rotation, Vector3 targetPosition)
    {
        ...
        {
            projectileBullet.Initialize(direction);
        }
        else
        {
